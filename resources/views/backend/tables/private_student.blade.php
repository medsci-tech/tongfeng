...
@section('js')
  ...
  <script>
      var data = {
          table_head: ['id', '手机号', '姓名', '邮箱', '医院' ,'科室', '职称', '报名时间'],
          data: [
              @foreach($students as $student)
              {
                  table_data: ['{{$student->id}}', '{{$student->phone}}','{{$student->name}}', '{{$student->email}}','{{$student->hospital}}', '{{$student->office}}', '{{$student->title}}', '{{$student->created_at}}']
              },
            @endforeach
          ],
          modal_data: '',
